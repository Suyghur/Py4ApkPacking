...
def replace_package_name(new_package_name) -> str:
    ...
    """
    manifest = Router.DECOMPILE_PATH + "/AndroidManifest.xml"
...
def merge_manifest(target_manifest, sdk_manifest):
    ...
    if not os.path.exists(target_manifest) or not os.path.exists(sdk_manifest):
        print("the manifest file is not exists.target_manifest:" + target_manifest + ";sdk_manifest:" + sdk_manifest)
        return False
...
def do_common_provider(new_package_name):
    provider_paths = Router.DECOMPILE_PATH + "/res/xml/provider_paths.xml"
    print("doCommonResXml provider_paths : " + provider_paths)
