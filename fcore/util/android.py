...
from fcore.util import java, command, fio
from fcore.util.router import Router


def decompile_apk() -> bool:
    # if os.path.exists(Router.DECOMPILE_PATH):
    cmd = "%s -jar -Xms2048m -Xmx2048m %s -v d -b -f %s -o %s" % (
        java.get_java_shell(), Router.APKTOOL231, Router.ORIGIN_APK, Router.DECOMPILE_PATH)
    return command.exec_command(cmd)


def compile_apk() -> bool:
    if os.path.exists(Router.COMPILE_PATH):
        cmd = "%s -jar -Xms2048m -Xmx2048m %s -v b -f %s -o %s" % (
            java.get_java_shell(), Router.APKTOOL231, Router.COMPILE_PATH, Router.OUTPUT_APK)
        return command.exec_command(cmd)
    else:
        return False
...
def split_dex():
    ...
    """
    smali_path = Router.DECOMPILE_PATH + "/smali"
    ##interaction multidex in mother apk
    for k in range(2, 20):
        temp_path = Router.DECOMPILE_PATH + "/smali_classes" + str(k)
    ...
    smali_temp = os.path.join(Router.DECOMPILE_PATH, "smali_temp")
    ...
    for f in all_files:
        ...
        if "Application" in f or "/cn/kkk/commonsdk" in f or "/cn/impl/common" in f or "/cn/impl/control" in f or "/android/support/multidex" in f \
            ...
            # 将文件移到smali
            target_path = f[0:len(Router.DECOMPILE_PATH)] + "/smali" + f[len(smali_temp):]
    ...
    for f in all_files:
        ...
        if total_fuc_num >= max_func_num or (total_fuc_num + this_fuc_num) >= max_func_num:
            ...
            target_path = f[0:len(Router.DECOMPILE_PATH)] + "/smali_classes" + str(curr_dex_index) + f[len(smali_temp):]
        else:
            total_fuc_num = total_fuc_num + this_fuc_num
            if curr_dex_index > 1:
...
def get_smali_method_count(smaliFile, allMethods):
    ...
    if not class_line.startswith(".class"):
        print(smaliFile + " not startswith .class")
        return 0
